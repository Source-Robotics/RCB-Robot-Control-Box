...
// VBUS Error Detection**
void check_vbus_error(uint32_t ms) {
  if (controller.filtered_VBUS2_mV > 25000) {
      // Start tracking time if not already started
      if (controller.vbus_high_start_time == 0) {
        controller.vbus_high_start_time = ms;
      }

      // Check if VBUS2 has been high for 5 seconds
      if ((ms - controller.vbus_high_start_time) >= VBUS_ERROR_TIME && !controller.vbus_error_flag) {
          //Serial.println("VBUS ERROR");
          controller.vbus_error_flag = true;  // Set flag to prevent repeated messages
          digitalWriteFast(RELAY_PIN, HIGH); // relay is OFF
      }
  } else {
      // Reset time tracking if VBUS2 drops below 25,000
      controller.vbus_high_start_time = 0;
      controller.vbus_error_flag = false;
  }
}


/// @brief Max Voltage is 62.7V, 180K and 10k resistor in series. For 3v3 on ADC pin we need 62.7V on input.
/// @brief 62700 mV = 4095 ADC ticks => (62700/4095) = 15.311 Voltage_mv = ADC_tick * 15.311
/// @param adc_value RAW ADC value
/// @return Voltage in mv
int Get_voltage_mA(int adc_value)
{

  int voltage = (int)(adc_value * 15.311);
  return voltage;
}
